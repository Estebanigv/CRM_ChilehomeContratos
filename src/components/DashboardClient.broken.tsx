...
const formatDate = (dateString: string): string => {
  ...
  try {
    ...
    if (new RegExp('^\\d{1,2}[\\/\\-]\\d{1,2}[\\/\\-]\\d{4}$').test(cleanDateString)) {
    ...
    else if (new RegExp('^\\d{4}[\\/\\-]\\d{1,2}[\\/\\-]\\d{1,2}$').test(cleanDateString)) {
...
const formatRUT = (rut: string): string => {
  ...
  const formattedNumbers = numbers.replace(new RegExp('\\B(?=(\\d{3})+(?!\\d))', 'g'), '.')
...
const formatPhone = (phone: string): string => {
  ...
  const cleanPhone = phone.replace(new RegExp('\\D', 'g'), '')
                  ...
                  clientesPaginados.map((cliente) => (
                    <tr key={cliente.id} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="py-4 text-gray-600">
                        {cliente.email || (
                          <span className="text-gray-400 italic">Falta correo</span>
                        )}
                      </td>
                      <td className="py-4">
        ...
        <div className="grid gap-4">
          {ventasPaginadas.map((venta: any) => (
            <div key={venta.id} className="group bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-all duration-200">
              <div className="p-5">
                {/* Línea superior: Nombre del cliente y estado */}
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border ${getEstadoStyle(venta.estado_crm || '')}`}>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
                  ...
                  <div className="bg-gray-50 rounded-lg p-3">
                    ...
                    <p className="text-sm font-bold text-gray-900">{formatProperCase(venta.ejecutivo_nombre)}</p>
                  </div>
  ...
  const handleGenerarContrato = async (venta: Venta) => {
    try {
      ...
      const response = await fetch('/api/crm/ventas', {
        ...
        body: JSON.stringify({
          ...
          contratoData: {
            ...
            vendedor: venta.ejecutivo_nombre,
      ...
      if (result.success) {
        ...
        setNotification({
          type: 'success',
          message: `¡Contrato ${result.contratoId} creado exitosamente para ${result.cliente}!`
        })
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <p><span className="font-medium">Vendedor:</span> {selectedVenta.ejecutivo_nombre}</p>
                    ...
                    <p><span className="font-medium">Modelo:</span> {selectedVenta.modelo_casa || 'No especificado'}</p>
      ...
      {showDetallesModal && selectedVenta && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-xl font-bold text-gray-900">Detalles Completos del Cliente</h3>
                <button
                  onClick={() => {
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-blue-50 rounded-lg p-5 border border-blue-100">
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="text-gray-900 font-medium">{selectedVenta.cliente_nombre}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="font-medium text-gray-600">RUT:</span>
                      <span className="text-gray-900 font-mono">{formatRUT(selectedVenta.cliente_rut)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="font-medium text-gray-600">Teléfono:</span>
                      <span className="text-gray-900">{formatPhone(selectedVenta.cliente_telefono)}</span>
                    </div>
                    <div className="flex flex-col gap-1">
                      <span className="font-medium text-gray-600">Dirección:</span>
                      <span className="text-gray-900 text-xs">{selectedVenta.direccion_entrega}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-green-50 rounded-lg p-5 border border-green-100">
                  ...
                  <div className="space-y-3 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-gray-900 font-medium">{selectedVenta.modelo_casa || 'No especificado'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="font-medium text-gray-600">Fecha Venta:</span>
                      <span className="text-gray-900">
                        {formatDate(selectedVenta.fecha_venta)}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="font-medium text-gray-600">Fecha Entrega:</span>
                      <span className="text-gray-900">{selectedVenta.fecha_entrega ? formatDate(selectedVenta.fecha_entrega) : 'Por definir'}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="font-medium text-gray-600">Estado:</span>
                      <span className={`inline-flex items-center px-3 py-1 rounded-md text-xs font-medium ${
                        selectedVenta.estado_crm?.toLowerCase().includes('validado') || selectedVenta.estado_crm?.toLowerCase().includes('completado')
                          ? 'bg-green-100 text-green-800 border border-green-200'
                          : selectedVenta.estado_crm?.toLowerCase().includes('rechazado') || selectedVenta.estado_crm?.toLowerCase().includes('cancelado')
                          ? 'bg-red-100 text-red-800 border border-red-200'
                          : selectedVenta.estado_crm?.toLowerCase().includes('proceso') || selectedVenta.estado_crm?.toLowerCase().includes('produccion')
                ...
                <div className="bg-purple-50 rounded-lg p-5 border border-purple-100">
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between">
                      <span className="font-medium text-gray-600">Ejecutivo:</span>
                      <span className="text-gray-900 font-medium">{selectedVenta.ejecutivo_nombre}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="font-medium text-gray-600">Supervisor:</span>
                      <span className="text-gray-900">{selectedVenta.supervisor_nombre || 'Sin supervisor'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="font-medium text-gray-600">Contrato Nº:</span>
                      <span className="text-gray-900 font-mono">{selectedVenta.numero_contrato || 'Sin generar'}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-gray-50 rounded-lg p-5 border border-gray-200">
                  ...
                  <div className="text-sm text-gray-700 bg-white p-3 rounded border">
                    {selectedVenta.observaciones_crm || 'Sin observaciones registradas'}
                  </div>
                </div>
              </div>
